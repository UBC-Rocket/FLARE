...
class XBeeResponse {
  public:
    ...
    /**
     * Returns the length of the frame data: all bytes after the api id, and
    ...
    /**
     * Call with instance of ZBExplicitRxResponse class only if getApiId() ==
     * ZB_EXPLICIT_RX_RESPONSE to populate response
     */
...
class XBeeAddress64 : public XBeeAddress {
  public:
    ...
    CONSTEXPR XBeeAddress64(uint32_t msb, uint32_t lsb)
        : _msb(msb), _lsb(lsb) {}
...
class XBeeRequest {
  public:
    ...
    /**
     * Sets the frame id.  Must be between 1 and 255 inclusive to get a TX
     * status response.
     */
     ...
     * common to all requests, whereas my definition of frame data is only the
     * API specific data.
     */
...
class XBee {
  public:
    XBee();
    /**
     * Reads all available serial bytes until a packet is parsed, an error
     * occurs, or the buffer is empty. You may call
     ...
     * quickly since it does not wait for serial data to arrive. You will want
     * to use this method if you are doing other timely stuff in your loop,
     * where a delay would cause problems. NOTE: calling this method resets the
     * current response, so make sure you first consume the current response
     */
    void readPacket();
    /**
     * Waits a maximum of <i>timeout</i> milliseconds for a response packet
     * before timing out; returns true if packet is read. Returns false if
     * timeout or error occurs.
     */
    bool readPacket(int timeout);
    /**
     * Reads until a packet is received or an error occurs.
     * Caution: use this carefully since if you don't get a response, your
     * Arduino code will hang on this call forever!! often it's better to use a
     * timeout: readPacket(int)
     */
    void readPacketUntilAvailable();
    /**
     * Starts the serial connection on the specified serial port
     */
    void begin(Hal::CustomSerial &serial);
     ...
     */
    void setSerial(Hal::CustomSerial &serial);
...
class XBeeWithCallbacks : public XBee {
  public:
     ...
     */
    void onResponse(void (*func)(XBeeResponse &, uintptr_t),
                    uintptr_t data = 0) {
     ...
     * the data parameter passed while registering the callback.
     */
    void onOtherResponse(void (*func)(XBeeResponse &, uintptr_t),
                         uintptr_t data = 0) {
    ...
    void onZBTxStatusResponse(void (*func)(ZBTxStatusResponse &, uintptr_t),
                              uintptr_t data = 0) {
        _onZBTxStatusResponse.set(func, data);
    }
    void onZBRxResponse(void (*func)(ZBRxResponse &, uintptr_t),
                        uintptr_t data = 0) {
        _onZBRxResponse.set(func, data);
    }
    void onZBExplicitRxResponse(void (*func)(ZBExplicitRxResponse &, uintptr_t),
                                uintptr_t data = 0) {
        _onZBExplicitRxResponse.set(func, data);
    }
    void onZBRxIoSampleResponse(void (*func)(ZBRxIoSampleResponse &, uintptr_t),
                                uintptr_t data = 0) {
        _onZBRxIoSampleResponse.set(func, data);
    }
    void onTxStatusResponse(void (*func)(TxStatusResponse &, uintptr_t),
                            uintptr_t data = 0) {
        _onTxStatusResponse.set(func, data);
    }
    void onRx16Response(void (*func)(Rx16Response &, uintptr_t),
                        uintptr_t data = 0) {
        _onRx16Response.set(func, data);
    }
    void onRx64Response(void (*func)(Rx64Response &, uintptr_t),
                        uintptr_t data = 0) {
        _onRx64Response.set(func, data);
    }
    void onRx16IoSampleResponse(void (*func)(Rx16IoSampleResponse &, uintptr_t),
                                uintptr_t data = 0) {
        _onRx16IoSampleResponse.set(func, data);
    }
    void onRx64IoSampleResponse(void (*func)(Rx64IoSampleResponse &, uintptr_t),
                                uintptr_t data = 0) {
        _onRx64IoSampleResponse.set(func, data);
    }
    void onModemStatusResponse(void (*func)(ModemStatusResponse &, uintptr_t),
                               uintptr_t data = 0) {
        _onModemStatusResponse.set(func, data);
    }
    void onAtCommandResponse(void (*func)(AtCommandResponse &, uintptr_t),
                             uintptr_t data = 0) {
        _onAtCommandResponse.set(func, data);
    }
    void onRemoteAtCommandResponse(void (*func)(RemoteAtCommandResponse &,
                                                uintptr_t),
                                   uintptr_t data = 0) {
    ...
    uint8_t waitFor(Response &response, uint16_t timeout,
                    bool (*func)(Response &, uintptr_t) = NULL,
                    uintptr_t data = 0, int16_t frameId = -1) {
        return waitForInternal(Response::API_ID, &response, timeout,
  ...
  private:
     ...
     */
    uint8_t waitForInternal(uint8_t apiId, void *response, uint16_t timeout,
...
class PayloadRequest : public XBeeRequest {
  public:
     ...
     * the payload array is 50 bytes and you only want the first 10 to be
     * included in the packet, set the length to 10. Length must be <= to the
     * array length.
     */
...
class ZBTxRequest : public PayloadRequest {
  public:
     ...
     */
    ZBTxRequest(const XBeeAddress64 &addr64, uint8_t *payload,
                uint8_t payloadLength);
    ZBTxRequest(const XBeeAddress64 &addr64, uint16_t addr16,
                uint8_t broadcastRadius, uint8_t option, uint8_t *payload,
                uint8_t payloadLength, uint8_t frameId);
    /**
     * Creates a default instance of this class.  At a minimum you must specify
...
/**
 * Represents a Series 2 TX packet that corresponds to Api Id:
...
class ZBExplicitTxRequest : public ZBTxRequest {
  public:
     ...
     * normal ZBTxRequest.
     */
    ZBExplicitTxRequest(XBeeAddress64 &addr64, uint8_t *payload,
                        uint8_t payloadLength);
    /**
     * Create a ZBExplicitTxRequest, specifying all fields.
     */
    ZBExplicitTxRequest(XBeeAddress64 &addr64, uint16_t addr16,
                        uint8_t broadcastRadius, uint8_t option,
                        uint8_t *payload, uint8_t payloadLength,
                        uint8_t frameId, uint8_t srcEndpoint,
...
class AtCommandRequest : public XBeeRequest {
  public:
    ...
    /**
     * Clears the optional commandValue and commandValueLength so that a query
     * may be sent
     */
