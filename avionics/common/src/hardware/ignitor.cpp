...
Ignitor::Ignitor(Pin ignitePin, Pin continuityPin, Pin continuityADCPin, bool hasRedundancyDelay)
    : ignitePin_(ignitePin), 
      continuityPin_(continuityPin),
      continuityADCPin_(continuityADCPin),
      hasRedundancyDelay_(hasRedundancyDelay) {
    /*init ignitor*/
...
void Ignitor::fire() {
    if (hasRedundancyDelay_) {
        Hal::sleep_ms(REDUNDANCY_DELAY);
    }
