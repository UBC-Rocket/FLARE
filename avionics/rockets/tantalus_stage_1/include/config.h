...
static State::Armed state_armed = State::Armed(
    StateId::POWERED_ASCENT, ARMED_LAUNCH_CHECKS, LAUNCH_THRESHOLD);

static State::PoweredAscent state_powered_ascent = State::PoweredAscent(
    StateId::PRE_AIR_START_COAST_TIMED, MOTOR_BURNOUT_CHECKS);

static State::PreAirStartCoastTimed state_prestage_coast =
    State::PreAirStartCoastTimed(
        POST_BURNOUT_COAST_TIME, PRESTAGE_MAX_ACCEPT_ALTITUDE,
        PRESTAGE_MIN_ACCEPT_ALTITUDE, PRESTAGE_MAX_ACCEPT_VERTICAL_ACCEL,
        PRESTAGE_MIN_ACCEPT_VERTICAL_ACCEL,
        PRESTAGE_MAX_ACCEPT_ANGLE_FROM_VERTICAL);

static State::AscentToApogee state_coast = State::AscentToApogee(
    APOGEE_CHECKS, MACH_LOCK_CHECKS, MACH_LOCK_VELOCITY_THRESHOLD);

static State::MachLock state_mach_lock =
    State::MachLock(MACH_UNLOCK_CHECKS, MACH_UNLOCK_VELOCITY_THRESHOLD);

static State::PressureDelay state_pres_delay =
    State::PressureDelay(APOGEE_PRESSURE_DELAY);

static State::DrogueDescent state_drogue =
    State::DrogueDescent(MAIN_DEPLOY_ALTITUDE, MAIN_DEPLOY_CHECKS);

static State::MainDescent state_main =
    State::MainDescent(std::chrono::milliseconds(LAND_CHECK_TIME_INTERVAL),
